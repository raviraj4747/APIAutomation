...
def runCommand(script, testresult):
    command = "aws ecs run-task --cluster converse-selenium --task-definition converse-selenium:5 --launch-type FARGATE --region us-east-1 --network-configuration \"awsvpcConfiguration={subnets=[subnet-0f9ed585c4d6d5f75],securityGroups=[sg-0d984628f590b7397],assignPublicIp=ENABLED}\" --overrides '{\"containerOverrides\": [{\"name\": \"converse-selenium\", \"command\": [\"pytest\"]}]}'&"
